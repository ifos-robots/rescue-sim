...
class VictimDetection:
    ...
    def detectionPipeline(self):
        ...
        for cameraImg, distanceValue, position in self.__getDistAndCamerasIterable():
            ...
            if isVictim == 1:
                isLetter, framedLetter = frameVictimLetter(
                    thresholdedImg, distanceValue)
                if isLetter:
...
def checkForDetectedSign(detectedSignsPositions, currentCoordinates, type):
    for sign, position in detectedSignsPositions:
        if (sign == type 
